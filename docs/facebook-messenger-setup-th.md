...
| `CLEANING_WEEKLY_REPORT_MAX_IMAGES` | `45` (ค่าสูงสุด) เพื่อลดโอกาสเกินเวลา/หน่วยความจำของ Apps Script |
...
## 8. เปิดแจ้งทันทีหลังบันทึก (แนะนำ)
...
## 9. ซิงก์ตารางเวรเพื่อแสดงรายชื่อผู้รับผิดชอบ

ข้อความเตือนจะอ่านตารางหมุนเวียนของสัปดาห์จริง ไม่ใช้การผูกชื่อกับเขตแบบถาวร:

1. ใช้ไฟล์ `MessengerNotifications.gs` เวอร์ชันล่าสุด แล้ว Deploy Web App เป็นเวอร์ชันใหม่ด้วย URL `/exec` เดิม
2. Merge/เผยแพร่หน้าเว็บเวอร์ชันที่มีการซิงก์ตารางเวร
3. เข้าระบบด้วยบัญชีแอดมิน เปิด **ตารางจัดเวรตรวจ** และเลือกเดือนปัจจุบัน
4. ตรวจรายชื่อและเขตของแต่ละสัปดาห์ แล้วกด **บันทึก** หรือ **เผยแพร่ตาราง**
5. ต้องเห็นข้อความสีเขียวว่า “ซิงก์รายชื่อผู้รับผิดชอบไปยังระบบแจ้งเตือนแล้ว”
6. ใน Apps Script รัน `previewCouncilDutyForToday` เพื่อตรวจชื่อผู้รับผิดชอบทั้ง 9 เขต
7. รัน `previewCleaningMessengerMessages` เพื่อตรวจรูปแบบข้อความก่อนส่งจริง

ระบบใช้เฉพาะตารางที่มีสถานะ **เผยแพร่แล้ว** หากตารางยังเป็นฉบับร่าง ข้อความจะแสดงว่า `ยังไม่พบตารางเวรที่เผยแพร่` เพื่อป้องกันการแจ้งชื่อผิด

ตัวอย่าง:

```text
• เขต 2 · ป.2
  ผู้รับผิดชอบ: กลุ่มที่ 4 (สภา04) — นักเรียนที่ผู้ดูแลกรอกในระบบ
```

ข้อความเวลา 09:00, 12:00, 15:00 และสรุปสิ้นวันจะแสดงรายชื่อรูปแบบนี้สำหรับทุกเขตที่ยังไม่ได้รายงาน
## 10. ขอรายงานและตารางผ่านแชท Messenger

เมื่อใช้ `MessengerNotifications.gs` เวอร์ชันล่าสุด ผู้ดูแลที่อนุมัติ PSID แล้วสามารถพิมพ์คำสั่งหา Page ได้โดยตรง:

| คำสั่ง | ผลลัพธ์ |
| --- | --- |
| `สรุปวันนี้` | จำนวนเขตที่ตรวจแล้ว เขตที่ยังไม่รายงาน และรายชื่อผู้รับผิดชอบ |
...
| `ตารางสัปดาห์นี้` | ตารางผู้ตรวจครบทั้ง 9 เขตของสัปดาห์ปัจจุบัน |
| `ตารางสัปดาห์หน้า` | ตารางของสัปดาห์ถัดไปเพื่อแจ้งนักเรียนล่วงหน้า |
| `ตารางสัปดาห์ 1` ถึง `ตารางสัปดาห์ 6` | ตารางของสัปดาห์ตามลำดับในเดือน |
...
ระบบจะตอบด้วย `messaging_type: RESPONSE` เพราะเป็นการตอบกลับข้อความที่ผู้ใช้เพิ่งส่งมา และอ่านเฉพาะตารางสถานะ **เผยแพร่แล้ว**

ค่าเริ่มต้นอนุญาตเฉพาะ PSID ใน `MESSENGER_ADMIN_RECIPIENT_IDS`:

- `MESSENGER_COMMANDS_ENABLED=true` เปิดการรับคำสั่ง
- `MESSENGER_COMMANDS_ADMIN_ONLY=true` จำกัดคำสั่งไว้เฉพาะผู้ดูแล
- หากต้องการอนุญาตผู้รับที่อยู่ใน `MESSENGER_RECIPIENT_IDS` ด้วย ให้เปลี่ยนเป็น `MESSENGER_COMMANDS_ADMIN_ONLY=false`

หลังคัดลอกโค้ดเวอร์ชันใหม่เข้า Apps Script ต้องสร้างเวอร์ชันใหม่ใน Deployment เดิม จากนั้นทัก Page ว่า `เมนู` เพื่อทดสอบ

รายงาน PDF ใช้เวลาประมาณ 1–2 นาที เพราะระบบสร้างเอกสารในงานเบื้องหลัง และเก็บไฟล์ไว้ในโฟลเดอร์ **Cleaning System Reports** ใน Google Drive ของบัญชีที่ Deploy เว็บแอป รายงานทางการจะรวมเฉพาะรายการที่ครูอนุมัติแล้ว

ก่อนใช้ PDF ครั้งแรก ให้รัน `initializeCleaningMessengerConfig` แล้วรัน `createDailyCleaningPdfNow` และ `createWeeklyCleaningPdfNow` อย่างละหนึ่งครั้ง เพื่ออนุญาตสิทธิ์ Google Docs, Google Drive และ URL Fetch จากนั้นสร้าง **เวอร์ชันใหม่** ใน Deployment เดิมโดยคง URL `/exec` เดิมไว้ หาก Meta แนบไฟล์ไม่สำเร็จ ระบบจะแจ้งข้อผิดพลาดโดยไม่เปิดเผยลิงก์ Drive ส่วนตัว
